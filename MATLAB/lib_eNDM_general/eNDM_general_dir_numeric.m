...
% Yuanxi's function: F(y) = d/(x+f)+e, fit d, e, f. Let alpha = 0.

%
% Output:
% y  =   eNDM predicted vectors (columns) at each time stamp

function [y,A] = eNDM_general_dir_numeric(x0,time_stamps,C,U,alpha,beta,s,d,e,f,a,b,p,solvetype,volcorrect)
if nargin < 12
    volcorrect = 0;
    if nargin < 11
        solvetype = 'numeric';
...
%     end
% else
%     t_span = 0:0.02:time_stamps(end);
%     [~,y] = ode45(@(t,x) (A*x + B+ (d./(x+e))),t_span,x0);    
%     y = y'; 
%     time_stamps_Loc = time_stamps/0.02+1;
%     y = y(:,time_stamps_Loc);
% end
...
if all(B == 0) && strcmp(solvetype,'analytic')
    ...
    t_span = 0:0.02:time_stamps(end);
%     [~,y] = ode45(@(t,x) (max(A*x + B+ (d./(x+0.0002)-e),0)),t_span,x0);    
    [~,y] = ode45(@(t,x) (max(A*x + B+ (d./(x+f)-e),0)),t_span,x0);
