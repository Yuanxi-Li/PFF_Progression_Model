...
%         Rvalues(jj) = corr(log(y(:,jj)+1),log(pathology(:,jj)+1), 'rows','complete');
    end
%     Rvalues = corr(log(y+1),log(pathology+1), 'rows','complete');
...
elseif strcmp(costfun_,'New_log_rval_sum')
    finiteinds = isfinite(sum(log(pathology),2));
    newxt = y;
    Rvalues = corr(reshape(log(y(finiteinds,:)),[],1),reshape(log(pathology(finiteinds,:)),[],1), 'rows','complete');
    %     Rvalues = corr(log(y+1),log(pathology+1), 'rows','complete');
