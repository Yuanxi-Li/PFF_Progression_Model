...
function [f,newxt,newpath] = objfun_eNDM_general_dir_costopts_numeric(param,seed_location,...
...
a_ = param(8:(n_types+7));
b_ = param((n_types+8):(2*n_types+7));
p_ = param((2*n_types+8):(3*n_types+7));

% Calculate predictions y with eNDM     
    % Solve eNDM; 
    ts(excltpts_costfun_) = [];
    pathology(:,excltpts_costfun_) = [];
    [y] = eNDM_general_dir_numeric(x0_,ts,C_,U_,alpha_,beta_,s_, d_, e_,f_,a_,b_,p_,solvetype_,volcorrect_);
