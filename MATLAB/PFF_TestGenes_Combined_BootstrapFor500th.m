function outputs_all = PFF_TestGenes_Combined_BootstrapFor500th(varargin)
% Conducted bootstrapping for each group,s 500th gene, combined effect
% By randomly permuting the elements of the gene expression values for 1,000 times.


% Parameters to fit
% param(1) = seed rescale factor, initial value
% param(2) = alpha, amplification/clearance
% param(3) = beta, spread parameter
% param(4) = s, directionality. 1 represents fully retrograde, 0 represents fully anterograde

% Defining LinR as Concordance Correlation Coefficient (CCC) (Lin 1989)
% LinRcalc = @(x,y) 2*corr(x,y)*std(x)*std(y)/(std(x)^2 + std(y)^2 + (mean(x) - mean(y))^2);

% 'BrainRegionReorderMat' in the file 'PFF_DataInput.mat' was used to reorder the gene expression array. (translate 424 brain regions' atlas into our atlas).

% Gene_expression_diag*C → gene expression effect on outgoing connections alone (outgoing);  
% C*Gene_expression_diag → gene expression effect on incoming connections alone (incoming);
% sqrt(Gene_expression_diag)*C*sqrt(Gene_expression_diag) → gene expression affects equally on both the outgoing and incoming connections simultaneously (combined) 

% Define defaults and set inputs
study_ = 'PFF_Progression'; 
costfun_ = 'log_LinR_sum'; % Loss function: Sum/Average CCC (LinR, 1989), log10-transformed 
solvetype_ = 'analytic';
volcorrect_ = 0;
exclseed_costfun_ = 0;
excltpts_costfun_ = [];
...
% Load in data (pathology data, gene expression data, and connectome)
if strcmp(ipR.study,'PFF_Progression')
    load([cd filesep 'PFF Data and Results\DataInput\PFF_DataInput.mat' ],...
        'PFF_Pathology_Data','PFF_Seed_Data','tpts', 'regvgene_mean', 'BrainRegionReorder_GeneExp','gene_names');
end

if strcmp(ipR.study,'PFF_Progression')
    load([cd filesep 'PFF Data and Results\DataInput\Mesoscale_Connectome.mat'],'Connectome_Data');
end

tic
gene_num = 3406; % 500th gene

Gene_expression_raw = regvgene_mean(:,gene_num);
BrainRegionReorderMat_New = [BrainRegionReorder_GeneExp;BrainRegionReorder_GeneExp+213];

[row_reorder, col_reorder] = size(BrainRegionReorderMat_New);
Gene_expression_Mat = nan(size(BrainRegionReorderMat_New));
...
for i = 1:row_reorder
    for j = 1:col_reorder
        if isnan(BrainRegionReorderMat_New(i,j))
            ...
            Gene_expression_Mat(i,j) = Gene_expression_raw(BrainRegionReorderMat_New(i,j));
        end
    end
end
Gene_expression = mean(Gene_expression_Mat,2,'omitnan');
Gene_expression(find(isnan(Gene_expression))) = mean(Gene_expression,'omitnan');

for Bootstrap_Num = 1:15000
    % Perform 15,000 times. 
    % Some cases can not be fit. Test 15,000 times and pick the first 1,000 results for further analyses.
    try

        C = Connectome_Data.raw;
        rng(Bootstrap_Num)
        Reorder_Perm(:,1) = randperm(410);
        Gene_expression_Reorder = Gene_expression(Reorder_Perm);

        Gene_expression_diag = diag(Gene_expression_Reorder);

        C = sqrt(Gene_expression_diag)*C*sqrt(Gene_expression_diag);   
        % gene expression affects equally on both the outgoing and incoming connections simultaneously (combined) 
        ...
        if ~logical(ipR.bootstrapping)
            ...
            time_stamps = tpts.PFF_Average;
            pathology = normalizer(PFF_Pathology_Data.PFF_Average,'log');
            seed_location = PFF_Seed_Data;
            ...
            NewRLog = corr(reshape(log(ynum(Logfiniteinds,:)),[],1),reshape(log(pathology(Logfiniteinds,:)),[],1), 'rows','complete'); % develop a new loss function, can also capture scale factor
            
            outputs.ndm.Full.results.data_means = mean(pathology,1,'omitnan');
            outputs.ndm.Full.results.Corrs = Rvalues;
            outputs.ndm.Full.results.Corrs_Mean = mean(Rvalues); % Average Pearson's R, log10-transformed, Michael Henderson et al. used this in Nat.Neurosci (2019)
            ...
            outputs.ndm.Full.results.New_R_Log = NewRLog; % develop a new loss function, can also capture scale factor
            outputs.ndm.Full.results.LogLinR = LogLinRvalues;
            outputs.ndm.Full.results.LogLinR_Mean = mean(LogLinRvalues); % Average CCC, log10-transformed, using this in the current paper
        ...
        % output results
        disp(['Bootstrap',num2str(Bootstrap_Num)])
        fldname = sprintf('Bootstrap_%d',Bootstrap_Num);
