function outputs = PFF_GlobalModel_Individual(varargin)

% Study_ = 'PFF_Progression', this script is for fitting individual mouse.
% Each mice has an individual seed rescale factor parameter, and share the
% same alpha, beta and s.


% Parameters to fit
% param(1:n_mice) = seed rescale factor, initial values
% param(n_mice+1) = alpha, amplification/clearance
% param(n_mice+2) = beta, spread parameter
% param(n_mice+3) = s, directionality
...
% Here, length(PFF mice) = 10
% PFF Mice, 1~7 seed rescale factors, 8 alpha, 9 beta, 10 s
param_init_ = [NaN(1,7),0,1,1];
ub_ = [ones(1,7),Inf,Inf,1];
lb_ = zeros(1,10);
lb_(8) = 0;
...
if ~logical(ipR.bootstrapping)
    ...
    time_stamps = tpts.PFF_Individual;
    pathology = normalizer(PFF_Pathology_Data.PFF_Individual,'log');
    seed_location = PFF_Seed_Data;

    U = zeros(size(C,1),1);
    [~,n_mice] = size(pathology);
    for i = 1:n_mice
        if isnan(ipR.param_init(i))
            ipR.param_init(i) = nansum(pathology(:,i))/nnz(seed_location); % heuristic default, study-dependent
        end
    end

    if ~logical(ipR.w_dir)
        ipR.param_init(end) = 0; 
        ipR.ub(end) = 1;
        ipR.lb(end) = 0;
    end
    ...
    objfun_handle = @(param) objfun_eNDM_general_dir_costopts_individual(param,...
    ...
    [~,n_mice] = size(pathology);
    x0_num = seed_location.*param_num(1:n_mice);
    alpha_num = param_num(n_mice+1);
    beta_num = param_num(n_mice+2);
    s_num = param_num(n_mice+3); % not fit if directionality is turned off
    a_num = param_num(n_mice+4); % not fit
    b_num = param_num(n_mice+5); % not fit
    p_num = param_num(n_mice+6); % not fit
    ynum = eNDM_general_dir_individual(x0_num,time_stamps,C,U,alpha_num,beta_num,s_num,a_num,b_num,p_num,ipR.solvetype,ipR.volcorrect);
    ...
    outputs.ndm.Full.results.LogCorrs = LogRvalues; % Average Pearson's R, log10-transformed, Michael Henderson et al. used this in Nat.Neurosci (2019)
    outputs.ndm.Full.results.LinR = LinRvalues;
    outputs.ndm.Full.results.LogLinR = LogLinRvalues; % Average CCC, log10-transformed, using this in the current paper
...
else % bootstrap method by Chaitali Anand et al., Sci.Rep 2022
    rng(0);
    for i = 1:ipR.niters
