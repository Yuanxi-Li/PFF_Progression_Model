function [outputs_all] = PFF_TestPartialConnectome_RemoveStrongest(varargin)
% Remove the elements of the connectome orderly by percentile, from strongest
% Ratio: from 1% to 99%, partial connectome
...
tic
Connectome_Ratio = 1:1:99;
for Connectome_Num = 1:length(Connectome_Ratio)
    try
        C = Connectome_Data.raw;
        RemoveNum = floor(numel(C)*(1-Connectome_Ratio(Connectome_Num)/100));
        [~,RemoveLoc] = maxk(C(:),RemoveNum);
        C(RemoveLoc) = 0;
        ...
        disp(['Ratio ',num2str(Connectome_Ratio(Connectome_Num)), ' Finished'])
        fldname = append('Ratio_',num2str(Connectome_Ratio(Connectome_Num)));
        fldname = strrep(fldname,'.','_');
        outputs_all.(fldname) = outputs.ndm.Full;
    catch
        continue
    end
end
SavePath = [cd filesep 'PFF Data and Results\'];
SaveFileName = append('RemoveStrongest.mat');
