function [outputs_all] = PFF_TestPartialConnectome_RatioFrom5to95(varargin)
% Removed random subsets of connections of various sizes, resampling the connectome 1,000 times per subset size
% Ratio: from 5% to 95%, partial connectome
...
if strcmp(ipR.study,'PFF_Progression')
    load([cd filesep 'PFF Data and Results\DataInput\PFF_DataInput.mat' ],...
        'PFF_Pathology_Data','PFF_Seed_Data','tpts', 'regvgene_mean', 'BrainRegionReorder_GeneExp');
end
...
Connectome_Ratio = 5:5:95; % Ratio: from 5% to 95%, partial connectome
for Ratio_Num = 1:length(Connectome_Ratio)
    outputs_all = struct();
    for BootstrapNum = 1:1200 % resampling the connectome 1,200 times per subset size
        % Some cases can not be fit. Test 1,200 times and pick the first 1,000 results for further analyses.
        try
            rng(BootstrapNum)
            C = Connectome_Data.raw;
            RemoveNum = floor(numel(C)*(1-Connectome_Ratio(Ratio_Num)/100));
            RandomOrder = randperm(410*410);
            RemoveOrder = RandomOrder(1:RemoveNum);
            C(RemoveOrder) = 0;
            C =  C/max(eig(C));
            ...
            if ~logical(ipR.bootstrapping)
                ...
                NewRLog = corr(reshape(log(ynum(Logfiniteinds,:)),[],1),reshape(log(pathology(Logfiniteinds,:)),[],1), 'rows','complete'); % develop a new loss function, can also capture seed scale factor
            ...
            %% Result output: 1,200 bootstraps.
            disp(['Bootstrap ',num2str(BootstrapNum), ' Finished'])
            fldname = sprintf('Bootstrap_%d',BootstrapNum);
            outputs_all.(fldname) = outputs.ndm.Full;
        catch
            continue
        end
    end
    disp(['Ratio ',num2str(Connectome_Ratio(Ratio_Num)/100), ' Finished'])
    SavePath = [cd filesep 'PFF Data and Results\']; % save the results for the testing ratio
    SaveFileName = append('Ratio_', num2str(Connectome_Ratio(Ratio_Num)),'.mat');
    save(append(SavePath, SaveFileName),'outputs_all')
end
toc
