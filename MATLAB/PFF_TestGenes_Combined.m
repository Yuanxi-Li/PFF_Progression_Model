function outputs_all = PFF_TestGenes_Combined(varargin)
% gene expression affects equally on both the outgoing and incoming connections simultaneously (combined effect) 
...
validStudy = @(x) ismember(x,{'PFF_Progression'});
...
tic
for gene_num = 1:length(gene_names)
    try
        C = Connectome_Data.raw;
        Gene_expression_raw = regvgene_mean(:,gene_num);
        BrainRegionReorderMat_Whole = [BrainRegionReorder_GeneExp;BrainRegionReorder_GeneExp+213]; %translate 424 brain regions' atlas into our 410-atlas 
        
        [row_reorder, col_reorder] = size(BrainRegionReorderMat_Whole);
        Gene_expression_Mat = nan(size(BrainRegionReorderMat_Whole));
        
        for i = 1:row_reorder
            for j = 1:col_reorder
                if isnan(BrainRegionReorderMat_Whole(i,j))
                    Gene_expression_Mat(i,j) = nan;
                else
                    Gene_expression_Mat(i,j) = Gene_expression_raw(BrainRegionReorderMat_Whole(i,j));
                end
            end
        end
        ...
        if ~logical(ipR.bootstrapping)
            ...
            outputs.ndm.Full.results.LogCorrs_Mean = mean(LogRvalues); % Average Pearson's R, log10-transformed, Michael Henderson et al. used this in Nat.Neurosci (2019)
